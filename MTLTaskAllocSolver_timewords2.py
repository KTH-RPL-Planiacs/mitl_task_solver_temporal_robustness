...
def generate_plan_mtl_wts(Demands,wts,horizon, return_optimum = False, suppress_output = False):
    ...
    #encodes the last time step with an active state
    for t in range(0,horizon):
        q_t = plp.LpVariable('q_t_'+str(t),cat='Integer')
        dict_vars['q_t_'+str(t)] = q_t
        qtilde_t = plp.LpVariable('qtilde_t_'+str(t),cat='Integer')
        dict_vars['qtilde_t_'+str(t)] = qtilde_t
        
        sum_b_s_t = plp.LpVariable('sum_b_s_t_'+str(t),cat='Binary')
        dict_vars['sum_b_s_t_'+str(t)] = sum_b_s_t
        opt_model += sum_b_s_t == plp.lpSum([dict_vars['b_s_'+state+'_t_'+str(t)] for state in wts.S])
        
    
    #encoding the last "active" state at a given time step
    opt_model += dict_vars['q_t_0'] == 0
    opt_model += dict_vars['qtilde_t_0'] == 0
    x_l = 0, horizon
    x_u = horizon
    for t in range(1,horizon):
        opt_model += dict_vars['q_t_'+str(t)] >= 0
        opt_model += dict_vars['q_t_'+str(t)] <= x_u*(1-dict_vars['sum_b_s_t_'+str(t)])
        opt_model += dict_vars['q_t_'+str(t)] <= dict_vars['q_t_'+str(t-1)] + 1
        opt_model += dict_vars['q_t_'+str(t)] >= dict_vars['q_t_'+str(t-1)] + 1 - (x_u * dict_vars['sum_b_s_t_'+str(t)])
        opt_model += dict_vars['qtilde_t_'+str(t)] == t - dict_vars['q_t_'+str(t)] 
        constraints += 5
    ...
    def model_phi(phi,t,opt_model,dict_vars):
        ...
        if isinstance(phi, MTLFormula.Predicate):
            ...
            proper_time = dict_vars['qtilde_t_'+str(t)]
    ...
    for phi, priority in Demands:
        ...
        for i in range(-MAX_RIGHT_TIME,0):
            try:
                qtilde_t = dict_vars['qtilde_t_'+str(i)]
            except KeyError:
                qtilde_t = plp.LpVariable('qtilde_t_'+str(i),cat='Integer')
                dict_vars['qtilde_t_'+str(i)] = qtilde_t
    ...
    print("\n\n TRAJECTORY")
    for t in range(0,horizon):
        print(t, [state for state in wts.S if dict_vars['b_s_'+state+'_t_'+str(t)].varValue == 1.0], dict_vars['q_t_'+str(t)].varValue, dict_vars['qtilde_t_'+str(t)].varValue, dict_vars['sum_b_s_t_'+str(t)].varValue)
    if not suppress_output: 
        print("Encoding time",encoding_time)
...
if __name__ == '__main__':
    ...
    e2 = MTLFormula.Always(MTLFormula.Predicate('M2'),15,18)
